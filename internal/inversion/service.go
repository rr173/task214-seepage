...
type Service struct {
	store *store.Store
	mu    map[string]*sync.Mutex // modelID -> *sync.Mutex (same-version serial solving)
}

// New builds an inversion service over the given store.
func New(s *store.Store) *Service { return &Service{store: s, mu: make(map[string]*sync.Mutex)} }
